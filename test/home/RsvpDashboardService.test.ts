...
describe("RsvpDashboardService", () => {
  ...
  describe("cancelRsvp", () => {
    it("cancels a going RSVP by using cancelAndPromoteNext", async () => {
      const { repository, service } = createHarness();
      ...
      expect(repository.cancelAndPromoteNext).toHaveBeenCalledWith(rsvp.id);
    ...
    it("maps cancelAndPromoteNext dependency failures to UnexpectedDependencyError", async () => {
      const { repository, service } = createHarness();
      repository.listRsvpsForUser.mockResolvedValue(
